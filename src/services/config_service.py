...
class Neo4jConfigurationService:
    ...
    async def stop_polling_session(
        ...
        """Stop polling session by configuration ID - delegated to PollingSessionService"""
        coordinator = await self._get_coordinator()
        polling_service = await coordinator.polling_sessions
        # Get active session for configuration
        active_sessions = await polling_service.get_active_sessions()
        config_session = None
        for session in active_sessions:
            if session.configuration_id == configuration_id:
        ...
        status = "force_stopped" if force_stop else "stopped"
        return await polling_service.end_polling_session(config_session.id, status)

    async def create_mcp_server_configuration(
        self, server_data: MCPServerConfigurationCreate
    ) -> MCPServerConfiguration:
        """Create MCP server configuration - delegated to MCPServerService"""
        return await self.create_mcp_server(server_data)
    ...
    async def list_mcp_server_configurations(
        ...
        """List MCP server configurations - delegated to MCPServerService"""
        return await self.list_mcp_servers(server_type, enabled_only, limit, offset)

    async def create_alert_from_xdr_data(self, alert_data: Dict[str, Any]) -> Alert:
        """Create alert from XDR data - delegated to AlertProcessingService"""
        return await self.store_enhanced_alert(alert_data)

    async def _get_secret_value(self, secret_name: str) -> Optional[str]:
        """Get secret value - delegated to SecretManagerService"""
        try:
            coordinator = await self._get_coordinator()
            return await coordinator.secrets.get_secret(secret_name)
        except Exception:
            return None
    ...
    async def get_configuration_statistics(
        ...
        """Get configuration statistics - delegated to PollingSessionService"""
        coordinator = await self._get_coordinator()
        polling_service = await coordinator.polling_sessions
        return await polling_service.get_polling_statistics(configuration_id)

    async def list_system_configurations(
        self, config_type: Optional[str] = None, limit: int = 100, offset: int = 0
    ) -> List[SystemConfiguration]:
        """List system configurations - placeholder for future implementation"""
        logger.warning(
            "list_system_configurations not yet implemented in decomposed services"
        )
        raise NotImplementedError("System configuration management not yet decomposed")
